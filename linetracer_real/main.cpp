...
int main()
{
    ...
    */
    ...
    int target, target2; // 따라갈 객체
    Point center(160, 45); // 초기 중심점 설정 (하단 영상 중앙)
    Point center2(480, 45); // 초기 중심점 설정 (하단 영상 중앙)
    int error, error2, mid;// 위치오차
    ...
    while (true) {
        ...
        get_target2(labeling, stats, center2, centroids, closest, target2, error2, cutthred);
        cvtColor(cutthred, cutthred, COLOR_GRAY2BGR);

        draw_target(labeling, stats, cutthred, target, center, centroids, target2, center2);
       ...
       mid = (error + error2) / 2;
        ...
        get_Velocity( lvel, rvel, mid, UNDER200, OVER200, RPM);
        if(mode) {
