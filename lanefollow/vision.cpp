...
void draw_target(int labeling, Mat stats, Mat& cutthred, int target, Point center, Mat centroids, int target2, Point center2){
    ...
    for (int i = 1; i < labeling; ++i) {
            ...
            if(i == target){
                ...
                rectangle(cutthred, Rect(p[0], p[1], p[2], p[3]), Scalar(0, 0, 255), 2);
                circle(cutthred, center2, 5, Scalar(0,0,255), -1);
             ...
             circle(cutthred, Point((center2.x + center.x)/2, 45), 5, Scalar(0,0,255), -1);
        }
}

void get_Velocity( int& lvel, int& rvel, int mid, double UNDER200, double OVER200, int RPM){
    if( mid >= 200  || mid <= -200 ){
        lvel= RPM - OVER200*mid;
        if(lvel < 0){
            lvel = 10;
        }
        rvel= -(RPM + OVER200*mid);
        if(rvel > 0){
            rvel = -10;
        }
    }
    else{
        lvel= RPM - UNDER200*mid;
        if(lvel < 0){
            lvel = 10;
        }
        rvel= -(RPM + UNDER200*mid);
        if(rvel > 0){
            rvel = -10;
        }
    }
